/** GCP perspective: one product, grouped by Business Domains (LAST_14). */
...
async function run(productName) {
  const filters = [
  ...
  return json.data?.perspectiveGrid?.data || [];
}

const products = process.argv.slice(2).length ? process.argv.slice(2) : ["Vertex AI", "AlloyDB"];
for (const p of products) {
  const rows = await run(p);
  console.log(JSON.stringify({ product: p, byBusinessDomain: rows }, null, 2));
}
