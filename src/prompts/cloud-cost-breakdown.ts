...
Steps:
1. **List perspectives**: Call harness_ccm_finops_list with resource_type="cost_perspective"${projectFilter} to see available cost perspectives
2. **Get cost summary**: Call harness_ccm_finops_get with resource_type="cost_summary"${projectFilter}${perspectiveFilter} to get overall cost totals
3. **Get cost breakdown**: Call harness_ccm_finops_get with resource_type="cost_breakdown"${projectFilter}${perspectiveFilter} to see costs broken down by service/cluster/environment
4. **Get trends**: Call harness_ccm_finops_get with resource_type="cost_timeseries"${projectFilter}${perspectiveFilter} to see cost trends over time
5. **Check anomalies**: Call harness_ccm_finops_list with resource_type="cost_anomaly"${projectFilter} to identify unusual cost spikes
